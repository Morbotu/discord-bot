...
module.exports = (message, blacklist) => {
    ...
    if (message.mentions.members.array().includes(message.member)) return message.reply("You can't ban yourself.").catch(console.error);
    if (message.member.hasPermission("ADMINISTRATOR")) {
        for (let i of message.mentions.members.array()) if (!blacklist.includes(i.user.tag) && !i.hasPermission("ADMINISTRATOR")) blacklist.push(i.user.tag);
